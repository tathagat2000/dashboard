...
class Dashboard2 extends Component {
          ...
          {res && res.data.length>0 && Object.keys(res.data[0]).map((val, index) => (
            ...
            >
              <div  style={{float: 'right', cursor: 'pointer'}}
                    onClick={(event) => {this.deleteColumn(event, index)}}
              > 
  ...
  deleteColumn = (event,dind) => {
    //let newTableData = this.state.tableData;

    let newTableData = this.state.tableData.map((rval)=> {
      return (
        rval.filter((cval,cind) => cind!==dind)
      )
    });
        ...
        if (reactLocalStorage.get("accessToken")) {
          axios
            ...
            .then((response) => {
              ...
              let newTableData = this.getTable(response);
